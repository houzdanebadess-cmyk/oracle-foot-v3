...
<style>
    ...
    .score-predict { font-size: 48px; color: #00f2fe; font-weight: 900; text-align: center; text-shadow: 0 0 15px #00f2fe; line-height: 1; }
    .score-live { font-size: 48px; color: #ff0055; font-weight: 900; text-align: center; text-shadow: 0 0 15px #ff0055; line-height: 1; }
    ...
    .footer { position: fixed; left: 0; bottom: 0; width: 100%; background: rgba(15, 12, 41, 0.95); color: #00f2fe; text-align: center; padding: 10px; border-top: 1px solid #00f2fe; z-index: 999; }
    .btn-share { background-color: #25d366 !important; color: white !important; padding: 8px 16px; border-radius: 20px; text-decoration: none; font-size: 13px; font-weight: bold; display: inline-block; }
</style>
...
def get_all_data():
    ...
    try:
        for lg in leagues:
            ...
            if 'matches' in m_d: all_m.extend(m_d['matches'])
        return all_s, all_m
...
if stats and matches:
    ...
    with t1:
        ...
        for m in today_m:
            ...
            msg = urllib.parse.quote(f"⚽ Mon prono sur ALPHA-ORACLE : {h_n} {p_h}-{p_a} {a_n}")
            
            # ICI LE CHANGEMENT : On utilise bien st.markdown avec unsafe_allow_html=True
            st.markdown(f"""
            <div class="card">
